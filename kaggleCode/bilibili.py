...
def load_data():
    ...
    """
    store = pd.read_csv('../input/store.csv')
    train_org = pd.read_csv('../input/train.csv', dtype={'StateHoliday': pd.np.string_})
    test_org = pd.read_csv('../input/test.csv', dtype={'StateHoliday': pd.np.string_})
...
def process_data(train, test, features, features_non_numeric):
    ...
    for col in set(features) - set(features_non_numeric) - \
        ...
        train[col] = scaler.fit_transform(np.array(list(train[col])).reshape(-1, 1))
